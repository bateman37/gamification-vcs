...
export default async function CharacterConfigPage({ params }: { params: { splitParticipantId: string } }) {
  ...
  return (
    <div className="space-y-8">
      <div>
        <Link href="/fichas" className="text-sm text-slate-600 underline hover:text-slate-900">
      ...
      <section id="resumen" className="space-y-3">
        ...
        <dl className="grid grid-cols-2 gap-4 rounded-lg border border-slate-200 bg-white p-4 text-sm sm:grid-cols-3">
            ...
            <dd className="font-medium">
              {character.profession ? (
                <>
                  ...
                  <span className="block text-xs font-normal text-slate-500">{formatPoweredKpis(character.profession)}</span>
          ...
          <div>
            <dt className="text-xs text-slate-500">Moneda actual</dt>
            <dd className="font-medium">{character.balance} creditos</dd>
          </div>
          <div>
            <dt className="text-xs text-slate-500">Total puntos KPI oficiales</dt>
            <dd className="font-medium">{character.totalOfficialKpiPoints.toFixed(2)}</dd>
          </div>
          <div>
            <dt className="text-xs text-slate-500">Total puntos de posicion</dt>
          ...
          {character.activeLocation && (
            <div className="col-span-2 sm:col-span-3">
              <dt className="text-xs text-slate-500">Localizacion activa esta semana</dt>
              <dd className="font-medium">
        ...
        {character.hasPublishedResults && (
          <p className="text-sm">
            <Link href={`/resultados?vista=por-split&split=${character.splitId}`} className="underline hover:text-slate-900">
      ...
      <section id="equipo" className="space-y-3 border-t border-slate-200 pt-6">
      ...
      <section id="inventario" className="space-y-3 border-t border-slate-200 pt-6">
        <h2 className="text-lg font-semibold">Inventario</h2>
        <InventoryPanel items={character.inventory} />
      </section>

      <section id="mercado" className="space-y-3 border-t border-slate-200 pt-6">
      ...
      <section id="historial" className="space-y-3 border-t border-slate-200 pt-6">
