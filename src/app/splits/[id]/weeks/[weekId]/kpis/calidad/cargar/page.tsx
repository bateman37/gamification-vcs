...
import { QualityUploadForm } from "./QualityUploadForm";

export default async function QualityUploadPage({
  params,
}: {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-xl font-semibold">Cargar Calidad</h1>
      ...
      {split.status !== "ACTIVE" ? (
        <p className="rounded-card border border-dashed border-border-strong px-4 py-6 text-center text-sm text-text-muted">
          Solo se puede cargar o sustituir Calidad en un split activo.
        </p>
      ) : (
        <QualityUploadForm splitId={split.id} weekId={week.id} />
      )}
    </div>
  );
}
