...
import { analyzeProductivityImportAction, confirmProductivityImportAction } from "@/server/actions/productivity.actions";
import {
  initialProductivityUploadState,
  type ProductivityUploadState,
} from "@/server/actions/productivity-action-state";
...
function formatRawOutcome(outcome: ProductivityKpiOutcomeView | null): string {
  if (!outcome || outcome.status !== "computed") return "-";
  return formatPoints(outcome.rawPoints ?? 0);
}

function PreviewSummary({ preview, weekId, splitId }: { preview: ProductivityPreview; weekId: string; splitId: string }) {
  return (
    <div className="space-y-4 rounded-lg border border-slate-200 bg-slate-50 p-4">
      ...
      {preview.existingImport && (
        <p className="text-sm text-amber-700">
          Ya existe una carga para esta semana ({preview.existingImport.originalFilename},{" "}
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-slate-200 bg-slate-100 text-slate-600">
              <tr>
                ...
                {preview.solutionHunterActive && (
                  <>
                    <th className="px-3 py-2 font-medium">Tickets resueltos</th>
                    <th className="px-3 py-2 font-medium">Cazador (sin limite / final)</th>
                  </>
                )}
                {preview.dataExplorerActive && (
                  <>
                    <th className="px-3 py-2 font-medium">Tickets comentados</th>
                    <th className="px-3 py-2 font-medium">Explorador (sin limite / final)</th>
                  </>
                )}
              </tr>
            </thead>
            <tbody>
              {preview.found.map((row) => (
                <tr key={row.participantId} className="border-b border-slate-100">
                  ...
                  {preview.solutionHunterActive && (
                    <>
                      <td className="px-3 py-2">{row.ticketsResolved}</td>
                  ...
                  {preview.dataExplorerActive && (
                    <>
                      <td className="px-3 py-2">{row.ticketsUpdatedWithComment}</td>
                      <td className="px-3 py-2">
...
export function ProductivityUploadForm({ splitId, weekId }: { splitId: string; weekId: string }) {
  const analyzeWithIds = analyzeProductivityImportAction.bind(null, splitId, weekId);
  const confirmWithIds = confirmProductivityImportAction.bind(null, splitId, weekId);
  const [analyzeState, analyzeFormAction] = useFormState<ProductivityUploadState, FormData>(
    analyzeWithIds,
    initialProductivityUploadState,
  );
  const [confirmState, confirmFormAction] = useFormState<ProductivityUploadState, FormData>(
    confirmWithIds,
    initialProductivityUploadState,
  );
  ...
  if (confirmState.ok && confirmState.confirmed) {
    return (
      <div className="space-y-3">
        <SuccessMessage>Carga confirmada correctamente.</SuccessMessage>
        <Link
          href={`/splits/${splitId}/weeks/${weekId}/kpis/productividad/comprobar`}
          className="inline-block text-sm text-slate-600 underline hover:text-slate-900"
        >
          Ir a Comprobar
        </Link>
